...
const StudentBreadcrumbs = ({location}) => {
  const [button, setButton] = useState(1)
    ...
    useEffect(() => {
      if (location === '/student/events/queries' || location === '/student/events/recommendations') {
        setButton(1)
      } else {
        setButton(-1)
      }
      ...
      <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 1 }} sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
          <Grid xs={6} sm={6} md={10} sx={{}}>
            <Breadcrumbs aria-label="breadcrumb" separator="/" maxItems={2} sx={{px: 2}}>
              ...
              sx={{ display: 'flex', alignItems: 'center' }}
              color="inherit"
              href="/student/events/recommendations"
              >
